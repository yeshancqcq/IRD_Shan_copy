...
nlp_clean_test <- clean_corpus(x = full_nlp_test, pubs = publications) #uses the clean_corpus.R function
...
names(test_group)[names(test_group) == '_gddid'] <- 'gddid'

test_group[, paste(wordIndex), by = gddid]
aggregate(word ~ gddid, test_group, c)
aggregate(.~gddid, test_group, paste, collapse = ",")
```
...
#extract dates below
is_date_test <- str_detect(full_nlp_test$word, "BP")

#full_nlp_test$word[is_date_test&ird_word][4]

date_range_test <- str_detect(full_nlp_test$word, 
                         "(\\d+(?:[.]\\d+)*),((?:-{1,2})|(?:to)),(\\d+(?:[.]\\d+)*),([a-zA-Z]+,BP),")

date_match_test <- str_match(full_nlp_test$word, 
                         "(\\d+(?:[.]\\d+)*),((?:-{1,2})|(?:to)),(\\d+(?:[.]\\d+)*),([a-zA-Z]+,BP),") %>% na.omit()

date_match_test2 <- str_extract(full_nlp_test$word, 
                         "(\\d+(?:[.]\\d+)*),((?:-{1,2})|(?:to)),(\\d+(?:[.]\\d+)*),([a-zA-Z]+,BP),") %>% na.omit()

number_test <- str_detect(full_nlp_test$word, 
                         ",(\\d+(?:[\\.\\s]\\d+){0,1}),.*?,yr,")

number_match_test <- str_match(full_nlp_test$word, ",(\\d+(?:[\\.\\s]\\d+){0,1}),.*?,yr,")%>% na.omit()

number_match_test2 <- str_match(full_nlp_test$word, ",(\\d+(?:[\\.\\s]\\d+){0,1}),yr,")%>% na.omit()

number_test2 <- str_detect(full_nlp_test$word, 
                         ",(\\d+(?:[\\.\\s]\\d+){0,1}),yr,")
...
```{r}
degmin_test <- str_match_all(nlp_test$word, dms_regex_test)
decdeg_test <- str_match_all(nlp_test$word, dd_regex_test)
date_test <- str_match_all(nlp_test$word, date_regex_test)
date_range_test <- str_match_all(nlp_test$word, date_range_regex_test)
```
...
things <- nlp_test %>% 
  filter(coord_pairs_test & age_test) %>% 
...
convert_dec <- function(x, i) {
  ...
  check_date <- function(input1, input2, index) {
    if(isTRUE(input1[index])){
      date_test_fun <- str_extract(nlp_test$word[index], 
                              "(\\d+(?:[.]\\d+)*),((?:-{1,2})|(?:to)),(\\d+(?:[.]\\d+)*),([a-zA-Z]+,BP)," )
    }
  ...
  point_pairs_test <- data.frame(sentence = nlp_test$word[i],
                            ...
                            date = check_date(date_range_test, number_test2, i),
...
for(n in 1:length(number_test2)){
  if(isTRUE(number_test2[n])){
    print(n)
  }
  n <- n+1
}
...
View(isTRUE(coord_pairs_test[78]))

coordinates_df_test <- coordinates_test %>% bind_rows %>% 
...
coordinates_df_test$doi <- coordinates_df_test$identifier %>% map(function(x) x$id) %>% unlist

leaflet(coordinates_df_test) %>% 
  addProviderTiles(providers$CartoDB.Positron) %>% 
  addCircleMarkers(popup = paste0('<b>', coordinates_df_test$title, '</b><br>',
                                  '<a href=https://doi.org/',
                                  coordinates_df_test$doi,'>Publication Link</a><br>',
                                  '<b>Sentence:</b><br>',
                                  '<small>',gsub(',', ' ', coordinates_df_test$sentence),
                                  '</small>'))
