...
test_group_new <- aggregate(.~gddid, test_group, paste, collapse = ",")
summary(test_group_new)
nlp_group_test <- test_group_new
names(nlp_group_test)[names(nlp_group_test) == 'gddid'] <- '_gddid'
...
coordinates_df_test_group_article <- coordinates_test %>% bind_rows %>% 
...
coordinates_df_test_group_article$doi <- coordinates_df_test_group_article$identifier %>% map(function(x) x$id) %>% unlist

leaflet(coordinates_df_test_group_article) %>% 
  addProviderTiles(providers$CartoDB.Positron) %>% 
  addCircleMarkers(popup = paste0('<b>', coordinates_df_test_group_article$title, '</b><br>',
                                  '<a href=https://doi.org/',
                                  coordinates_df_test_group_article$doi,'>Publication Link</a><br>',
                                  '<b>Sentence:</b><br>',
                                  '<small>',gsub(',', ' ', coordinates_df_test_group_article$sentence),
                                  '</small>'))
