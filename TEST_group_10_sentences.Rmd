...
short_table_test <- nlp_group_test  %>% 
  filter(1:nrow(test_group_10s) %in% 1) %>%
  str_replace('-LRB-', '(') %>% 
  str_replace('-RRB-', ')') %>% 
...
coordinates_df_test_group_10s <- coordinates_test %>% bind_rows %>% 
...
coordinates_df_test_group_10s$doi <- coordinates_df_test_group_10s$identifier %>% map(function(x) x$id) %>% unlist

leaflet(coordinates_df_test_group_10s) %>% 
  addProviderTiles(providers$CartoDB.Positron) %>% 
  addCircleMarkers(popup = paste0('<b>', coordinates_df_test_group_10s$title, '</b><br>',
                                  '<a href=https://doi.org/',
                                  coordinates_df_test_group_10s$doi,'>Publication Link</a><br>',
                                  '<b>Sentence:</b><br>',
                                  '<small>',gsub(',', ' ', coordinates_df_test_group_10s$sentence),
                                  '</small>'))
